...
def parse_args():
    parser = argparse.ArgumentParser(description='Process all of plinder and store in zarr')
    parser.add_argument('--data', type=str, required=True, help="Path to systems directory")
    parser.add_argument('--output', type=str, required=True, help="Path to zarr store")
    parser.add_argument('--pocket_cutoff', type=int, required=False, default=8, help="Angstrom cutoff for pocket extraction")
    parser.add_argument('--num_systems', 
                       type=int,
                       required=False,
                       help='Number of systems to process, ')
