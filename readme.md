...
For information on pharmacophore file formats and how to prepare pharmacophore inputs, see [docs/pharmacophores.md](docs/pharmacophores.md).
...
For large-scale sampling — generating thousands of replicates for a single target, or evaluating a checkpoint across hundreds of dataset systems — use the **distributed sampling pipeline**. It distributes work across a SLURM cluster with automatic chunking, dependency chaining, metrics computation, and result aggregation.

Two modes are available:
- **CLI mode**: Parallelize replicates across GPU jobs for a single target (uses the `omtra` CLI under the hood)
- **Dataset mode**: Parallelize systems across GPU jobs for benchmark evaluation (uses `docking_eval.py` under the hood)

```bash
# Example: generate 1000 ligands for a protein target across 10 GPU jobs
python -m omtra_pipelines.distributed_sampling.launch_pipeline \
  --config my_target.yaml --site cluster --dry-run
```

See the [Distributed Sampling Pipeline README](omtra_pipelines/distributed_sampling/README.md) for full documentation, including step-by-step guides, configuration reference, and troubleshooting.
