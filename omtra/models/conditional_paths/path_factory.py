...
def get_conditional_path_fns(task_cls, conditional_path_config: DictConfig) -> Dict[str, Tuple[str, Callable]]:
    """
    ...
    for modality in task_cls.modalities_generated:
        try:
            conditional_path_fn_key = conditional_path_config[task_cls.name][modality.name]['type']
        except KeyError:
            conditional_path_fn_key = task_cls.conditional_paths[modality.name]['type']

        try:
            conditional_path_params = conditional_path_config[task_cls.name][modality.name]['params']
        except KeyError:
            conditional_path_params = task_cls.conditional_paths[modality.name].get('params', {})
        ...
        conditional_path_fn_output[modality.name] = (conditional_path_fn_key, conditional_path_fn)
